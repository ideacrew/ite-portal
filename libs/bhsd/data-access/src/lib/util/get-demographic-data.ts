...
export const convertExtractToDemographics = (submission: ExtractSubmissionResponse): ExtractSubmissionDemographics => {
...
export const getGenderBreakdown = (records: ExtractRecordValidation[]): DemographicData[] => {
  const genders = records.map((record) =>
...
export const getRaceBreakdown = (records: ExtractRecordValidation[]): DemographicData[] => {
  const races = records.map((record) =>
    record.payload.race
  );
...
export const getEthnicityBreakdown = (records: ExtractRecordValidation[]): DemographicData[] => {
  const ethnicities = records.map((record) =>
    record.payload.ethnicity
  );
...
export const getDemographics = (values: Array<string | null | undefined>, options: humanMap[]): DemographicData[] => {
  ...
  const missingCount = findValueCount(values, null) + findValueCount(values, undefined)
  const dataMissing = {label: 'Missing Data', count: missingCount, percent: (Math.round((missingCount / values.length) * 100))}
  if (dataMissing.count > 0) {
    demos.push(dataMissing)
  }
  const dataValues = options.map((option) => option.dataValue)
  const nonValidCount = findNonValidCount(values, [...dataValues, null, undefined]);
  const dataNonValid = {label: 'Non-Valid Data', count: nonValidCount, percent: (Math.round((nonValidCount / values.length) * 100))}
  if (dataNonValid.count > 0) {
    demos.push(dataNonValid)
  }
  return demos.sort((a: DemographicData, b: DemographicData) => b.count - a.count)
}

const findValueCount = (possibles: Array<string | Date | null | undefined>, value: string | null | undefined): number => possibles.filter((possible) => possible === value).length
const findNonValidCount = (possibles: Array<string | null | undefined>, values: Array<string | null | undefined>): number => possibles.filter((possible) => !values.includes(possible)).length

const getAgeBreakdown = (records: ExtractRecordValidation[]): DemographicData[] => {
  ...
  const ageOptions: humanMap[] = [
    ...
    {dataValue: 'Missing DoB', text: 'Missing DoB'},
    {dataValue: 'Invalid DoB', text: 'Invalid DoB'}
  ];
...
const findAgeCount = (ages: Array<number | 'Invalid DoB' | 'Missing DoB'>, ageRange: string): number => {
  if (ageRange === 'Invalid DoB' || ageRange === 'Missing DoB') {
    ...
    return ages.filter((age) => ( typeof age !== 'string') && (age >= min && age <= max)).length
  }
};
...
const getAges = (dateString: string): number | 'Invalid DoB' => {
  ...
  if (date_regex1.test(dateString) || date_regex2.test(dateString)) {
    ...
    return Math.floor((Date.now() - new Date(birthDate).getTime()) / 3.155_76e+10)
  } else {
    return 'Invalid DoB'
  }
